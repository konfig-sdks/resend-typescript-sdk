...
import { readableStreamToString, ResendError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new ResendError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new ResendError(e, e.response?.data)
                }
                if (innerError instanceof ResendError) {
                    // Got "ResendError" from the above try block
                    throw innerError;
                }
